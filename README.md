
## - 👋 Hi, I’m aluvse golang, web developer
## - 🌱I'm seeking collaborations and opportunities to work with companies that are dedicated to helping others and contributing to a better world.
## - 📫 How to reach me: [![TG](https://img.shields.io/badge/Telegram-2CA5E0?style=for-the-badge&logo=telegram&logoColor=white)](https://t.me/aluvse)


  <h3>always remember why you started ( ⚡, 🇰🇷, 🇺🇸 )</h3>
  <p> 
    <img  alt="USA🇺🇸" title="<3" height="265px" src="https://github.com/aluvse/aluvse/blob/main/assets/outside-weather.gif">
    <img  alt="Canada" title="<3" height="265px" src="./assets/hurricane-canada.gif">
    <img  alt="zorro⚡" title="<3" height="215px" src="https://github.com/aluvse/aluvse/blob/main/assets/39e352ea5caf1247fb29969a7cc8d4d2.gif">
    <img  alt="South Korea🇰🇷" title="<3" height="265px" src="https://media1.giphy.com/media/v1.Y2lkPTc5MGI3NjExYThhNjlvaW0zbGpsMDNrc3JiOG1tNDNnbDFiMG90bHMxZnd5aTNzZiZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/274oCYbGZUQKUEW5rH/giphy.gif">
  </p>


<details close>
  <summary>gitHub statistics 📊</summary>
  <br>
    <p>
      <a href="https://github.com/aluvse" width="100%">
        <img alt="Top Langs" height="165px" src="https://github-readme-stats.vercel.app/api/top-langs/?username=aluvse&layout=compact">
        <img alt="GitHub Stats" height="165px" src="https://github-readme-streak-stats.herokuapp.com?user=aluvse&border_radius=4">
      </a>
    </p>
</details>
<!---
aluvse/aluvse is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
<details close>
  <summary> Goals </summary>
  <br>
    <p>
# 🚀 Дорожная карта достижений  

## ✔ Выполнено:  
- [x] Начать стримить  

## 🚵 Задача дня:  
- [ ] Golang conf  
- [ ] Чекнуть DWH  

## 🎯 Задача недели:  
- [ ] Написать микросервис Go  
- [ ] Написать todo-golang  
- [ ] Написать DWH по туториалу  

## 🔥 Стрики:  
- **Golang каждый день**: 0  
- **Стримы**: 0  
- **Leetcode**: 0  

## 📌 На очереди:  

### 💻 Технологии & Разработка  
- [ ] Leetcode: 0 → 100 → 500 → 1000  
- [ ] Golang: Junior → Middle → Senior  
- [ ] MLOps: Написать систему на Golang  
- [ ] OpenSource: Внести вклад в проекты  
- [ ] Dev-выступления: Поделиться знаниями  
- [ ] Тимлид: Возглавить команду  
- [ ] DWH Engineering: Освоить Data Warehousing  
- [ ] Web-сервис: Запустить свой проект  
- [ ] Steam RPG: Выпустить игру  
- [ ] Кросс-платформенное приложение  
- [ ] GitHub Streak: 365 дней кодинга 🔥  

### 🎥 Контент & Творчество  
- **Стриминг (Twitch)**:  
  - [ ] Подписчики: 0 → 100 → 2000 → 12.5K  
  - [ ] Сабы: 0 → 20 → 200  
- [ ] Boosty: 100 сабов  
- **YouTube**:  
  - [ ] Документальные фильмы  
  - [ ] Технический/креативный контент  
- **Музыка**:  
  - [ ] Выпустить трек → клип → альбом  

### 🛠 Оборудование  
- [ ] MacBook Pro 16" 2TB  
- [ ] Видеозахват: Профессиональная карта  
- [ ] Камера: Плавный 4K+  
- [ ] Микрофон: Студийное качество  

### ⚡ Мечты  
- [ ] Робо-самурай: Собрать крутого бота  
- [ ] Марс: Билет в один конец 🚀  
- [ ] Дом: Построить свое пространство  

### 💪 Спорт & Здоровье  
- [ ] Тренировки 3-7 раз/неделю (силовые/кардио)  
- [ ] Бег/велоспорт → полумарафон/марафон  
- [ ] Плавание → 2+ км без остановки  
- [ ] Воркаут → 20+ подтягиваний, 50+ отжиманий  
- **Цели**:  
  - [ ] 10% body fat  
  - [ ] Жим 100+ кг  
  - [ ] Становая 150+ кг  
  - [ ] Подтягивания +25 кг  
  - [ ] 5 км за <20 минут  
- **Здоровье**:  
  - [ ] Чекапы 1-2 раза в 2025  

---  
**🔥 Главное — системность, дисциплина и кайф от процесса!**  
**🔋 Здоровье — база всех достижений!**  
    </p>
</details>
